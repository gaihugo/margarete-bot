...
const execute = async (client, message, args, authr, avatar) => {
  ...
  if (something) {
    ...
    song = await memeRandom(something);
  }
  if (!something) {
    // Caso não tenha algo
  ...
  if (!message.member.voice.channel) {
    message.channel.send("Loga num canal de audio o fela da puta", {
      tts: true
    });
    return;
  }
...
const playSong = async (client, message, song) => {
  ...
  if (!queue) {
    ...
    queue.dispatcher = await queue.connection.play(
      await ytdl(`https://www.youtube.com/watch?v=${song}`),
      { filter: "audioonly" }
    );
